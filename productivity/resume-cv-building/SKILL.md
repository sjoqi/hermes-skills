...
## Step 4: deliver
- **Default output: .docx** (universally ATS-parsable — see Step 3c / ats-best-practices §5). Generate via the **pdf-generation** skill's docx path, or — when python-docx / LibreOffice / OfficeCLI are unavailable — the zero-dependency generator `scripts/generate_ooxml_docx.py` (stdlib `zipfile` + WordprocessingML, no third-party libs; validated as a clean, table-free, ATS-safe DOCX).
